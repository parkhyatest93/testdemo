...
(() => {
  ...
  class ShopifySubscriptionsWidget {
    ...
    // Wrap window.fetch once to detect /cart/add responses and redirect to checkout on success
    enableCartAddRedirect() {
      ...
      window.fetch = async (...args) => {
        ...
        try {
          ...
          if (requestUrl instanceof Request) requestUrl = requestUrl.url;
          const url = new URL(requestUrl, location.origin);
          if (url.pathname.includes('/cart/add') && response.ok) {
            // small delay to allow cart to update, then redirect to checkout
            setTimeout(() => {
    ...
    handleVariantChange() {
      //this.handleSellingPlanFormVisibility(); we will show all of them
      this.updateSellingPlanInputsValues();
      this.listenToSellingPlanFormRadioButtonChange();
    }
