...
function formatLines(
  ...
  return nodesFromEdges(graphqlLines.edges).map((lineRaw) => {
    const line = lineRaw as typeof lineRaw & {
      customAttributes?: { key: string; value: string }[];
    };

    const quantity = line.quantity ?? 1; // keep quantity normalized if needed later
    const currency = line.currentPrice.currencyCode;

    // Original product unit price stored as custom attribute "productBasePrice"
    const baseAttr =
      line.customAttributes?.find((attr) => attr.key === 'productBasePrice') ??
      null;

    // Fallback to the contract's currentPrice if attribute is missing
    const productUnitPrice = baseAttr?.value ?? line.currentPrice.amount;

    // Total subscription line price from Shopify (already includes discounts)
    const subscriptionLinePrice = line.lineDiscountedPrice.amount;
    ...
    return {
      ...
      // For the UI: original product price per unit
      currentPrice: {
        amount: productUnitPrice,
        currencyCode: currency,
      },

      // For the UI: total subscription line price (discounted)
      lineDiscountedPrice: {
        amount: subscriptionLinePrice,
        currencyCode: line.lineDiscountedPrice.currencyCode ?? currency,
      },
    } as SubscriptionLine;
  });
}
