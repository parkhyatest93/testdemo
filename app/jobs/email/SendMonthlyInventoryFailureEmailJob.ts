...
export class SendMonthlyInventoryFailureEmailJob extends Job<{}> {
  async perform(): Promise<string[]> {
    ...
    await findActiveBillingSchedulesInBatches(async (batch) => {
      this.logger.info(
        `Processing SendMonthlyInventoryFailureEmailJob for ${batch.length} shops`,
      );
        ...
        batch.map(async (billingSchedule) => {
          ...
          if (isMonthlyNotificationFrequency) {
            ...
            this.logger.info(
              ...
              'Shop added for monthly inventory failure email',
            );
            return true;
          }
      ...
      this.logger.info(
        ...
        'Processed shops for SendMonthlyInventoryFailureEmailJob',
      );
    });
