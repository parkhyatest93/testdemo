...
// Query: Fetch a single Billing Cycle by date selector (used to detect "skipped today")
const GET_BILLING_CYCLE_BY_DATE_QUERY = `
...
function writeTextLog(entry: unknown, fileName = "custom_log_sub.log") {
  ...
  // Build a daily file name: <base>-YYYY-MM-DD<ext>
  const now = new Date();
  ...
  const dailyFileName = `${parsed.name}-${dayKey}${parsed.ext || ".log"}`;
  const filePath = path.join(logDir, dailyFileName);

  // If an object is passed, format as JSON
...
async function updateNextBillingDate(admin: any, contractId: string, currentNextBillingDate: string, billingIntervalDays: number) {
  // Berechne die neue nextBillingDate: HEUTE + billingIntervalDays
  // (nicht alte nextBillingDate + Intervall, da die alte oft veraltet/falsch ist)
  const today = DateTime.utc().startOf('day');
  const newNextBillingDate = today.plus({ days: billingIntervalDays }).toISO();

  // Verwende subscriptionContractUpdate um die nextBillingDate zu aktualisieren
  const mutation = `
  ...
  try {
    writeTextLog({
      ...
      calculation: `Today (${today.toISO()}) + ${billingIntervalDays} days = ${newNextBillingDate}`,
      note: "Using subscriptionContractUpdate - Shopify will auto-calculate next billing date based on delivery policy"
    });

    // WICHTIG: subscriptionContractUpdate ohne Änderungen triggert Shopify, die nextBillingDate neu zu berechnen
    // Dies ist besser als manuelles Setzen, da Shopify die Billing Cycles korrekt verwaltet
    const response = await admin.query({
      data: {
        query: mutation,
        variables: {
    ...
    const result = response.body?.data?.subscriptionContractUpdate;
...
async function createBillingAttempt(admin: any, contractId: string, originTime: string) {
  ...
  try {
    ...
    const normalizedOriginTime = date.toISOString().slice(0, 10);
    const idempotencyKey = `rebill-${contractId}-${normalizedOriginTime}`;
    const response = await admin.query({
      data: {
        query: mutation,
        variables: {
...
// Helper: Resolve a single billing cycle for a specific date (selector: { date })
async function getBillingCycleByDate(admin: any, contractId: string, dateIso: string) {
  try {
    const response = await admin.query({
      data: {
        query: GET_BILLING_CYCLE_BY_DATE_QUERY,
        variables: {
          contractId,
          date: dateIso,
        },
    ...
    if (data?.errors) {
      writeTextLog({
        message: `Error fetching billing cycle by date for ${contractId}`,
        date: dateIso,
        errors: data.errors,
      });
      return null;
    ...
    return data?.data?.subscriptionBillingCycle ?? null;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    writeTextLog({
      message: `Error fetching billing cycle by date for ${contractId}`,
  ...
  try {
    ...
    do {
      ...
      const contractsThisPage = edges
        ...
        .filter((contract) => {
          return true; // allowedIds.has(contract.id);
        });
      ...
      for (const contract of contractsThisPage) {
        ...
        // continue; // NOTE: remove this line to enable processing
        ...
        // ✅ WICHTIGE PRÜFUNG: Contract muss EXAKT das Intervall überfällig sein!
        // Beispiel: Bei 28 Tage Intervall → nur bei exakt 28 Tagen Überfälligkeit abrechnen
        const remainder = daysOverdue % intervalDays;
        const isMultipleOfInterval = daysOverdue > 0 && daysOverdue % intervalDays === 0;
        if (!isMultipleOfInterval) {
          writeTextLog({
            message: `Skipping ${contract.id} - not at exact interval overdue`,
            contractId: contract.id,
            daysOverdue: daysOverdue,
            intervalDays: intervalDays,
            nextBillingDate: nextBillingDate,
            note: `This job only processes contracts that are EXACTLY ${intervalDays} days overdue. Other overdue contracts will be handled by a different job.`
          });
          results.push({
            id: contract.id,
            success: false,
            error: `Not at exact interval (${daysOverdue} days overdue, but needs to be exactly ${intervalDays} days)`
          });
          failureCount++;
          continue;
        }
        ...
        writeTextLog({
          message: `✅ Contract is EXACTLY ${intervalDays} days overdue - proceeding`,
          contractId: contract.id,
          daysOverdue: daysOverdue,
          intervalDays: intervalDays,
          nextBillingDate: nextBillingDate
        ...
        // ✅ PRE-CHECK: If customer skipped TODAY's billing cycle, do NOT create a billing attempt
        // We resolve the billing cycle for TODAY via selector: { date } and respect `skipped/status`.
        const todaySelector = DateTime.utc().startOf("day").toISO();
        const todayCycle = await getBillingCycleByDate(admin, contract.id, todaySelector as string);

        if (todayCycle?.skipped === true || todayCycle?.status === "SKIPPED") {
          writeTextLog({
            message: `Skipping ${contract.id} - billing cycle for TODAY is skipped`,
            contractId: contract.id,
            todaySelector,
            cycleIndex: todayCycle?.cycleIndex,
            expectedDate: todayCycle?.billingAttemptExpectedDate,
            status: todayCycle?.status,
            skipped: todayCycle?.skipped,
          });

          results.push({
            id: contract.id,
            success: false,
            error: "Billing cycle skipped today (customer skip)",
          });
          failureCount++;
          continue;
        }
        ...
        writeTextLog({
          ...
          daysOverdue: daysOverdue,
          intervalDays: intervalDays,
          note: `Creating billing attempt for contract that is exactly ${intervalDays} days overdue`
        });
          ...
          if (statusResult.completed && !statusResult.error) {
            ...
            if (nextBillingUpdateResult?.success) {
              writeTextLog({
                ...
                calculation: `Today + ${intervalDays} days`
              });
            } else {
              writeTextLog({
